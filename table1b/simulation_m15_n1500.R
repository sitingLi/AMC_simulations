...
weights_result1 = weights_result2 = matrix(NA, 45, simu_times)
wqs_weights = matrix(NA, 45, simu_times)


for (j in 1:simu_times){
  print(paste("Number of the currently executing iteration:", j))
  # generate data
  data=simu(size = 1500, nvariate = 15, prob = c(0.4, 0.3, 0.2, 0.1), mu=c(1, 2, 3, 4), 
  ...
  newdata <- dummy_obs[,16:60] # X1-X15 (categorized by amc) are transformed to dummy variables
  ...
  obs <- dummy_obs[, 16:60]  # X1-X15 (categorized into quantiles) are transformed to dummy variables
